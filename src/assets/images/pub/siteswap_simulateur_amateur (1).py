...
# J'avais essayé d'enregistrer les séquences dans un fichier, mais je comprenais pas trop json.dump avec indent, ça bugguait
# import json
# def save_to_file(seq_dict):
#     with open("sequences.json", "w") as f:
#         json.dump(seq_dict, f)  # ça plantait ici quand y’avait des int... ou alors je passais mal les noms ?
#         # du coup j’ai laissé tomber, juste dict en mémoire.

# À un moment, j’ai voulu afficher une ligne genre "===================" en ASCII pour faire joli mais c’était moche
# print("=" * 80)
# puis après je l’ai viré... trop bruyant à force

# J’ai aussi tenté un “mode débug” qui affiche tout ce que le programme fait mais... ça polluait tout donc supprimé.
# DEBUG = True
# if DEBUG:
#     print("DEBUG:", sequence, sum(sequence))

# Animation plus réaliste ? J’ai voulu utiliser math.sin mais le résultat était pire qu’avant 😭
# import math
# for t in range(steps):
#     y_offset = math.sin(t / steps * math.pi) * (base_y - y_max)
#     y = base_y - y_offset
#     canvas.coords(ball, x - rayon, y - rayon, x + rayon, y + rayon)
#     window.update()
#     time.sleep(0.015)
# Résultat : la balle montait et descendait deux fois, j’ai pas compris pourquoi

# Je voulais faire une interface avec des boutons tkinter… mais j’ai abandonné quand j’ai vu les grid() et pack()
# def fake_interface():
#     btn = tk.Button(window, text="OK", command=lambda: print("clic"))
#     btn.grid(row=0, column=0)
# Trop compliqué à rendre propre avec juste une console derrière.