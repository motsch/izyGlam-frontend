...
export class InvoiceService {
  ...
  private async generateStyledInvoice(order: any): Promise<jsPDF> {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('Date du service :', 20, 90);
    doc.setFont('helvetica', 'normal');
    doc.text(this.formatDate(order.start), 70, 90);

    // CALCULS FIXES
    const ttc = parseFloat(order.price); // Prix total TTC
    const tvaRate = 20; // taux fixe à 20%
    const commission = parseFloat(order.commission || '0'); // HT
    const shopHT = parseFloat(order.shopEarnings || '0'); // HT
    const totalHT = +(shopHT + commission).toFixed(2);
